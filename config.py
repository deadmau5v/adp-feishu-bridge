...
class FeishuConfig:
    ...
    def from_env(cls) -> "FeishuConfig":
        return cls(
            ...
            debug_raw_event=os.getenv("BRIDGE_DEBUG_RAW_EVENT", "false").lower()
            in ("true", "1", "yes"),
...
class BridgeConfig:
    ...
    def from_env(cls) -> "BridgeConfig":
        ...
        def _split_lines(val: str) -> list[str]:
            return [
                ln.strip() for ln in val.replace("\\n", "\n").splitlines() if ln.strip()
            ]
        ...
        return cls(
            ...
            streaming_send=os.getenv("BRIDGE_STREAMING_SEND", "false").lower()
            in ("true", "1", "yes"),
    ...
    def validate(self) -> list[str]:
        ...
        if self.trigger_mode not in ("at", "always"):
            errors.append(
                f"BRIDGE_TRIGGER_MODE 仅支持 at / always，当前值: {self.trigger_mode}"
            )
        return errors
